...
import time
def procesar():
    global tipoPeriodo,mes,anio,temporal_
    ...
    #Variables globales
    global Consecutivo_
    global token,tiempoexpiracion_
    global tiempo_
    tiempo_ = 0
    token = ""
    Token_ = AccesToken()
    ...
    res = resp['data']
    Id_ = res[0]['ID']
    os.mkdir('ArchivosComprobantes')
    Consecutivo_ = res[0]['Consecutivo_nomina']
    Consecutivo_ = int(Consecutivo_)
    # print("Empieza")
    Estado_ = "[Enviada a Pago,Pagada]".replace(" ","%20")
    ...
    if(len(Periodo_) < 5):
        ...
        lock = threading.Lock()
        # Creacion de hilos
        hilo1 = threading.Thread(target=HilosPorDF(Valores1,tipoPeriodo,lock))
        hilo2 = threading.Thread(target=HilosPorDF(Valores2,tipoPeriodo,lock))
        hilo3 = threading.Thread(target=HilosPorDF(Valores3,tipoPeriodo,lock))
        hilo4 = threading.Thread(target=HilosPorDF(Valores4,tipoPeriodo,lock))
        hilo5 = threading.Thread(target=HilosPorDF(Valores5,tipoPeriodo,lock))
        hilo6 = threading.Thread(target=HilosPorDF(Valores6,tipoPeriodo,lock))
        hilo7 = threading.Thread(target=HilosPorDF(Valores7,tipoPeriodo,lock))
        hilo8 = threading.Thread(target=HilosPorDF(Valores8,tipoPeriodo,lock))
        # Iniciar hilos
        hilo1.start()
        hilo2.start()
        hilo3.start()
...
def NuevaLineaComprobantesContables(Consecutivo_,FechaFinal_,CodigoCuentaContable_,Identificacion_,Descripcion_,CodigoCentro_,Debito_,Credito_,IDPeriodo_,TipoDeComprobante_,_Nomina):
    ...
    for periodosX_ in Periodo_:
        ...
        for NominaX_ in Nomina_:
            ...
            if(NominaEmpleado['Neto'].sum() > 0 ):
                ...
                FilaAgregar = NuevaLineaComprobantesContables(Consecutivo_,FechaFinal_,CodigoCuentaContable_,Identificacion_,Descripcion_,CodigoCentro_,Debito_,Credito_,IDPeriodo_,TipoDeComprobante_,IDNomina_)
            ...
            Datos_ = json.dumps(Datos_)
            url_ = "https://creator.zoho.com/api/v2/hq5colombia/compensacionhq5/report/Reporte_de_Nominas_Desarrollo/"+ str(NominaX_)
            header = {"Authorization":"Zoho-oauthtoken "+Token_, "Access-Control-Allow-Origin": "*"} 
            r = requests.put(url_,data=Datos_,headers=header)
        #Cierra Nomina
    #Cierra periodo
            ...
            if( SumaComprobantes < 499 ):
                ...
                ComprobanteTotal_.to_excel("ArchivosComprobantes/Comprobante"+ str(IDPeriodo_)+ "_" + str(Consecutivo_) +".xlsx",index=False)
                # Llamar API de zoho para actualizar datos del consecutivo
                Token_ = AccesToken()
                # Realizar consulta
                Datos_ = {"data": {"Anio":anio,"Mes":mes,"Tipo_de_periodo":tipoPeriodo,"Temporal":temporal_ }}
                Datos_ = json.dumps(Datos_)
                ...
                Id_ = res['ID']
                # Realizar consulta
                Datos_ = {'file': open('ArchivosComprobantes/Comprobante' + str(IDPeriodo_)+ '_' + str(Consecutivo_) +'.xlsx', 'rb')}
